/*
Usage:
$ APP_ADDRESS=[dissent app addr] VOTE_ID=[vote id for analysis] npx truffle exec --network mainnet scripts/vote-stats.js
...
module.exports = async cb => {
  const dissent = Dissent.at(appAddress)
  ...
  const [
    ...
    votingPower,
    execScript,
  ] = await dissent.getVote(voteId)

  const token = MiniMeToken.at(await dissent.token())
  const decimals = await token.decimals()

  const fromBlock = snapshotBlock.toString()

  dissent.CastVote({ voteId }, { fromBlock }).get(async (err, votes) => {
    if (err) {
      throw err
    }
